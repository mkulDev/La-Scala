...
const Description = ({ align, title, main, body, btn, btnId }: PropsType) => {
  return (
    <div className={`flex flex-col font-display items-center  ${align === 'left' ? 'lg:items-start' : align === 'right' ? 'lg:items-end' : 'lg:items-center'} md:mb-[4vh]`}>
      {title && <p className={`font-body flex flex-col text-sm md:text-2xl`}>{title} </p>}
      {title && <img src={spoon} alt='fork-img' className={`flex flex-col w-[50px] my-3 ${align === 'right' ? 'rotate-180' : 'rotate-0'}`} />}
      <h2
